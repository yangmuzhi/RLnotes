...
class double_table_qlearning:
    ...
    def e_greedy_choose_action(self, state):
        ...
        if np.random.uniform() < self.epsilon:
            # 当出现最大有多个情况时，应该随机选择
            action_value = self.Q1[str(state)] + self.Q2[str(state)]
    ...
    def update(self, obs):
        ...
        if np.random.uniform() < 0.5:
            action_value = self.Q1[str(s_next)]
            self.Q1[str(s)] += self.learning_rate * (r +
                            self.discount_factor * self.Q2[str(s_next)][
                            np.random.choice(np.where(self.Q1[str(s_next)] == action_value)[0])
                            ] - self.Q1[str(s)][a])
        else:
            action_value = self.Q1[str(s_next)]
            self.Q2[str(s)] += self.learning_rate * (r +
                            self.discount_factor * self.Q1[str(s_next)][
                            np.random.choice(np.where(self.Q2[str(s_next)] == action_value)[0])
                            ]- self.Q2[str(s)][a])
...
a = 0
s_next = s
q_learning.Q1[str(s)] += q_learning.learning_rate * (r +
        q_learning.discount_factor *
        q_learning.Q2[str(s_next)][
        np.random.choice(np.where(q_learning.Q1[str(s_next)] == 0)[0])
        ]
        - q_learning.Q1[str(s)][a]
        )


if __name__ == "__main__":
    env = Maze()
    q_learning = double_table_qlearning(
            n_actions = 4,
            env = env,
            epsilon=0.8,
            )
