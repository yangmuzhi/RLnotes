...
class deep_qlearning:
    ...
    # the memory size should less than a constant : memory_size 
    def add_to_sampling_pool(self):
        if len(self.sampling_pool) > self.memory_size:
            self.sampling_pool = self.sampling_pool[1:] 
    ...
    def update(self):
        ...
        #action_value is list
           ...
           # q_s_a = self.Q_eval_net(torch.tensor(list(stat_action)) )
        
            #q_s_a = self.Q_eval_net(torch.tensor(list(stat_action)) )       
            q_s_a =  reward + self.discount_factor * max(self.Q_target_net(torch.tensor(list(next_stat_action)))) 
